...
// Load .env before reading vars — if this runs too early, `env.NEXT_PUBLIC_*: ''` can clobber real values.
require('dotenv').config({ path: path.join(__dirname, '.env.local') });
require('dotenv').config({ path: path.join(__dirname, '.env') });

const treasuryFromPublic = (process.env.NEXT_PUBLIC_TREASURY_ADDRESS || '').trim();
const treasuryFromSecret = (process.env.TREASURY_ADDRESS || '').trim();
const treasuryForBrowser = treasuryFromPublic || treasuryFromSecret;

const nextConfig = {
  // Never set NEXT_PUBLIC_TREASURY_ADDRESS to "" — that overrides .env and breaks client deposits.
  ...(treasuryForBrowser
    ? {
        env: {
          NEXT_PUBLIC_TREASURY_ADDRESS: treasuryForBrowser,
        },
      }
    : {}),
