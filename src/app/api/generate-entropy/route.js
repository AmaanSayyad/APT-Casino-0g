...
const ENTROPY_NETWORK = normalizeEntropyNetworkKey(
  process.env.NEXT_PUBLIC_ENTROPY_NETWORK || process.env.NEXT_PUBLIC_CASINO_ENTROPY_NETWORK
);
const ENTROPY_NET_CFG = PYTH_ENTROPY_CONFIG.getNetworkConfig(ENTROPY_NETWORK);

const PYTH_ENTROPY_ADDRESS =
  process.env.NEXT_PUBLIC_PYTH_ENTROPY_CONTRACT || ENTROPY_NET_CFG?.entropyContract;
const ENTROPY_RPC = process.env.NEXT_PUBLIC_ORACLE_RPC_URL || ENTROPY_NET_CFG?.rpcUrl;
const TX_EXPLORER_BASE = (
  process.env.NEXT_PUBLIC_ORACLE_TX_EXPLORER || ENTROPY_NET_CFG?.explorerUrl || ''
).replace(/\/$/, '');
const PYTH_CHAIN_SLUG = process.env.NEXT_PUBLIC_PYTH_CHAIN_SLUG || '';
...
export async function POST(request) {
  try {
    ...
    // Create provider
    const provider = new ethers.JsonRpcProvider(ENTROPY_RPC);
    ...
    const entropyProof = {
      ...
      network: ENTROPY_NETWORK,
      explorerUrl: PYTH_CHAIN_SLUG
        ? `https://entropy-explorer.pyth.network/?chain=${encodeURIComponent(PYTH_CHAIN_SLUG)}&search=${encodeURIComponent(String(requestId))}`
        : `https://entropy-explorer.pyth.network/?search=${encodeURIComponent(String(requestId))}`,
      txExplorerUrl: `${TX_EXPLORER_BASE}/tx/${tx.hash}`,
      timestamp: Date.now(),
    ...
    return Response.json({
      ...
      entropyProof: {
        ...
        network: ENTROPY_NETWORK,
        explorerUrl: PYTH_CHAIN_SLUG
          ? `https://entropy-explorer.pyth.network/?chain=${encodeURIComponent(PYTH_CHAIN_SLUG)}`
          : 'https://entropy-explorer.pyth.network/',
        txExplorerUrl: TX_EXPLORER_BASE ? `${TX_EXPLORER_BASE}/` : '',
        timestamp: Date.now(),
