    ...
    <div className="bg-[#1A0015] rounded-xl p-6 mb-10 shadow-xl">
        ...
        <div className="flex flex-wrap gap-8 justify-center md:justify-end">
          ...
          <div className="flex flex-col items-center">
            <GradientBorderButton>
              <Link href="/bank" className="text-white no-underline">
                Treasury
              </Link>
            </GradientBorderButton>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-[#250020] rounded-lg p-3">
          ...
          <div className={`text-lg font-medium ${stats.priceChange24h != null && Number(stats.priceChange24h) > 0 ? 'text-green-400' : stats.priceChange24h != null ? 'text-red-400' : 'text-white/70'}`}>
            {stats.loading ? (
              <div className="h-5 w-16 bg-white/10 animate-pulse rounded"></div>
            ) : stats.priceChange24h == null ? (
              '--'
            ) : (
              `${Number(stats.priceChange24h) > 0 ? '+' : ''}${stats.priceChange24h}%`
            )}
          </div>
        </div>
      </div>
