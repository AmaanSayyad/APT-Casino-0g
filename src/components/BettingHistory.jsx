...
import { oracleTxUrl } from '@/utils/oracleExplorer';

const BettingHistory = ({ history }) => {
  const openOracleTx = (txHash) => {
    const url = oracleTxUrl(txHash);
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', mt: 0.5 }}>
                ...
                <Tooltip title={bet.vrfDetails?.transactionHash ? 'VRF Verified - Click to view' : 'Result'}>
                  <Box
                    ...
                    onClick={() => bet.vrfDetails?.transactionHash && openOracleTx(bet.vrfDetails.transactionHash)}
                  >
                    {bet.roll}
                ...
                <Box sx={{ mt: 1, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                  ...
                  <Tooltip title="View transaction">
                    <Chip
                      ...
                      color="success"
                      icon={<Verified />}
                      onClick={() => openOracleTx(bet.vrfDetails.transactionHash)}
                      sx={{ 
                        fontSize: '0.6rem', 
                        height: 20,
                        cursor: 'pointer',
