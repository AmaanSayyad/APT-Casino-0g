...
import { oracleTxUrl, pythEntropyDashboardSearchUrl, isRealTxExplorerUrl } from '@/utils/oracleExplorer';
                      ...
                      {item.entropyProof ? (
                        <div className="text-xs text-gray-300 font-mono">
                          ...
                          <div className="flex gap-1 mt-1">
                            {(isRealTxExplorerUrl(item.entropyProof.txExplorerUrl) ||
                              oracleTxUrl(item.entropyProof.transactionHash)) && (
                              <button
                                onClick={() =>
                                  window.open(
                                    isRealTxExplorerUrl(item.entropyProof.txExplorerUrl)
                                      ? item.entropyProof.txExplorerUrl
                                      : oracleTxUrl(item.entropyProof.transactionHash),
                                    '_blank'
                                  )
                                }
                            ...
                            {item.entropyProof.transactionHash && (
                              <button
                                onClick={() =>
                                  window.open(
                                    pythEntropyDashboardSearchUrl(item.entropyProof.transactionHash),
                                    '_blank'
                                  )
                                }
                            ...
                            {/* 0G Network Button */}
                            {(() => {
                              const ogLog = getOGNetworkLog(item);
                              console.log('🔍 Wheel GameHistory - item ID:', item.id, 'has ogNetworkLog:', !!item.ogNetworkLog, 'ogLog result:', ogLog);
