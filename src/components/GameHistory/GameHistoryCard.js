...
const GameHistoryCard = ({ game, gameType }) => {
  ...
  const openEntropyExplorer = (txHash) => {
    if (txHash) {
      window.open(pythEntropyDashboardSearchUrl(txHash), '_blank');
    }
  };
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                {(isRealTxExplorerUrl(game?.entropyProof?.txExplorerUrl) || oracleTxUrl(game?.entropyProof?.transactionHash)) && (
                  <Button
                    size="small"
                    variant="outlined"
                    ...
                    onClick={() =>
                      window.open(
                        isRealTxExplorerUrl(game?.entropyProof?.txExplorerUrl)
                          ? game.entropyProof.txExplorerUrl
                          : oracleTxUrl(game?.entropyProof?.transactionHash),
                        '_blank'
                      )
                    }
                    sx={{
