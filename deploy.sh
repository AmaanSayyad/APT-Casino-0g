...
# Frontend build + Vercel deploy (0G EVM–focused app). Legacy Aptos/Move steps removed.
...
# Move/Aptos contracts removed from this flow — use Hardhat (npm run deploy:*) for EVM/0G.
compile_contracts() {
    print_warning "Skipping Move/Aptos compile (project is 0G EVM–focused)."
}

deploy_contracts() {
    print_warning "Skipping Move/Aptos deploy. Use Hardhat scripts for EVM contracts."
}
